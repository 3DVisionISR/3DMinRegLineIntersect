...
int main(){
    ...
    for (int iter = 0 ; iter < pow(10,powIter) ; iter++){ 
        ...
        Matrix<double, 4,4> transGT = getGTTransformations<double>(verbose);
        lCPairs.push_back(genLineCorrPair<double>(transGT));
        lCPairs.push_back(genLineCorrPair<double>(transGT));
	    ...
	    // run solver
        vector<Matrix<double, 4,4>> estTrans = solver2M<double>(ptPairs,plPairs,lPairs,lCPairs);
        // vector<Matrix<double, 4,4>> estTrans = solver3L1Q<double>(ptPairs,plPairs,lPairs);
        ...
        for(int iiter= 0; iiter < estTrans.size(); iiter++){
            ...
            //if(!isnan(estT(1,1))){
                ...
                cout << "Estimated Transformation:" << endl << estT << endl;
                //if (verbose) 
                cout << "Error (frob norm): " << (transGT-estT).norm() << ", time: " << duration << endl;
            //}
        }         
    }
}
